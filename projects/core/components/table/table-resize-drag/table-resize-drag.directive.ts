...
export class TableResizeDragDirective<T> {
  ...
  readonly cuTableResizeDrag = fromEvent(
    ...
    switchMap(() => {
      const {width, right} = this.cell.elementRef.nativeElement.getBoundingClientRect();
  ...
  constructor(
    ...
    @Inject(forwardRef(() => TableCellHeaderComponent)) readonly cell: TableCellHeaderComponent<T>,
  ) { }
