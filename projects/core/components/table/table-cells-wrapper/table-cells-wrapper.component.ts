...
export abstract class TableCellsWrapperComponent<T> implements AfterViewInit {
  ...
  get cellsList(): TableCellComponent<T>[] {
    return Array.from(this.cells ?? []).concat(
      Array.from(this.headCells ?? [])
    );
  }

  ngAfterViewInit() {
    this.cellsList.forEach((cell, index) =>
      cell.setColumn(this.tableComponent.columns[index] as keyof T)
    );
  }
}
